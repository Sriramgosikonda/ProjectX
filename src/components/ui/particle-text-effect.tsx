...
export function ParticleTextEffect({ words = DEFAULT_WORDS }: ParticleTextEffectProps) {
  ...
  const animate = () => {
    ...
    // Handle mouse interaction - optimized
    ...
    // Auto-advance words - reduced frequency for performance
    frameCountRef.current++
    if (frameCountRef.current % 900 === 0) { // Slower word changes for better performance
  ...
  useEffect(() => {
    ...
    canvas.width = Math.min(1920, window.innerWidth * devicePixelRatio)
    canvas.height = Math.min(1080, window.innerHeight * devicePixelRatio)
